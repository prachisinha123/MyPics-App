...
export class Pics extends Component {
    ...
    async componentDidMount(){
        // let url = "https://picsum.photos/v2/list&pageSize=20";
    ...
    handlePrevClick = async ()=>{
         ...
         let url = `https://picsum.photos/v2/list&page=${this.state.page-1}&pageSize=20`;
         let data = await fetch(url);
         console.log(data);
         this.setState({
  ...
  render() {
    ...
    return (
      <div className="container my-3">
        <h2>MyPics-Headlines</h2>
        {/* {this.state.articles.map((element)=>{console.log(element)})} */}
        <div className="row">
        {this.state.articles.map((element)=>{
            return   <div className="col-md-4" key ={element.url}>
               <PicsItem title={element.Title} author={element.author} imageUrl={element.download_url}
                picUrl={element.url}/>  
           </div>
        })}
         <div className="container d-flex justify-content-between">
         <button disables={this.state.page<=1} type="button" class="btn btn-dark"onClick={this.handlePrevClick}>&larr; Previous</button>
